"""Expectimax+UCB1 heuristic -- THIRD-PARTY PUBLIC AGENT, used as an offline opponent only.

Source notebook : https://www.kaggle.com/code/raunakdey07/pok-mon-tcg-advanced-heuristic-agent
Licence         : Apache-2.0 (public Kaggle notebook)
publicScore     : 796.8 (read 2026-07-23)
...
UCB1_EXPLORATION_CONSTANT = 0.5  # Tune this to balance exploration vs exploitation
MCTS_ITERATIONS = 15
...
Path(_os.path.join(_SCRATCH, "deck.csv")).write_text("\n".join(map(str, DECK)) + "\n")
...
DECK_PATH = _os.path.join(_SCRATCH, "deck.csv")
...
def evaluate_state(obs):
    """Calculates a strictly heuristic value for the current board state."""
    st = obs.current
    ...
    for p in [op.active[0] if op.active else None] + list(op.bench):
        ...
        elif p.id == 121: op_dmg = 130 if assumed_energies >= 2 else 0 
...
def SEARCH_ALGO(obs_dict, obs):
    ...
    try:
        ...
        # Phase 2: UCB1 Exploitation vs Exploration
        while time.time() - t0 < SEARCH_TIME_BUDGET:
            ...
            for a in candidates:
                ...
                ucb = norm_avg + UCB1_EXPLORATION_CONSTANT * math.sqrt(math.log(total_visits) / visits[a])
                if ucb > best_ucb:
                    best_ucb = ucb
                    best_a = a

...
DECK = ([MAKUHITA]*2 + [HARIYAMA]*2 + [LUNATONE]*2 + [SOLROCK]*3 +
        [RIOLU]*3 + [MLUCARIO]*4 +
        [DUSK_BALL]*4 + [SWITCH_ITEM]*2 + [PPP]*4 + [GONG]*4 + [POKE_PAD]*4 +
        [HERO_CAPE] + [BOSS]*2 + [CARMINE]*4 + [LILLIE]*4 +
        [GRAVITY_MOUNTAIN]*2 + [F_ENERGY]*13)
...
W = {
    ...
    'ppp_for_ko':       6600,
    'attach':           5000,
    'attach_defer':     3600,   # below search(4200)/draw(4000); attach last in turn
    'tool':             4600,
    'retreat_to_atk':   6000,
    'attack':           1000,
    'end':              0.5,
    ...
    def score(o):
        ...
        if ot == ATTACH:
            ...
            if cid == F_ENERGY:
                ...
                # v31L sequencing: the once-per-turn manual attach should happen
                # AFTER search/draw within the turn (play Dusk Ball / Poke Pad /
                # Gong / Lunar Cycle first, then attach with full information).
                # probability_v2 (933.8) does this as a matter of course -- the
                # dominant MAIN disagreement class was ATTACH:Energy -> PLAY:item.
                # Base is set below search_item(4200)/draw_supporter(4000); the
                # target-quality term (which Pokemon) is scaled to a tiebreak so
                # a "bank on bench" attach never leapfrogs development. An attach
                # that ENABLES the active to attack this turn still ranks itself
                # highest among attaches via the is_act bonus.
                pr = _energy_priority(tgt, my, plan, is_act)
                return W['attach_defer'] + pr * 0.25
            return W['attach_defer']
...
def _choose(obs):
    ...
    if ctx == CTX_ATTACH_FROM or ctx == CTX_ATTACH_TO:
        # Aura Jab attaches Basic F from the discard to a benched Pokemon. The
        # target can be addressed either via inPlayArea/inPlayIndex OR as a plain
        # CARD via area/index -- the first draft only handled the former, so in
        # the CARD case _attach_target returned None, every bench Pokemon tied at
        # 100, and the tie broke on array order onto Lunatone (a non-attacker).
        # Disagreement mining vs probability_v2 (933.8) flagged this as the
        # single highest-rate context (74.2% disagreement): the strong pilot
        # routes discard energy onto Makuhita/Hariyama/Solrock. Resolve the
        # target Pokemon however it is addressed, then score by attacker need.
        plan = Plan()
        scores = []
        my_active = _active(my); bench = _bench(my)
        for o in opts:
            tgt = _attach_target(o, my_active, bench)
            if tgt is None:
                # CARD-addressed target: pull the actual Pokemon out of the board
                area = o.get('area')
                if area == AREA_ACTIVE:
                    tgt = my_active
            ...
            if tgt is None:
                ...
                is_act = tgt is my_active
                scores.append(_energy_priority(tgt, my, plan, is_act))
        order = sorted(range(len(opts)), key=lambda i: -scores[i])

...
DECK = ([ABRA]*4+[KADABRA]*4+[ALAKAZAM]*3+[DUNSPARCE]*3+[DUDUNSPARCE]*3+
        ...
        [BATTLE_CAGE]*3+[NIGHT_STRETCHER]+[DAWN]*4+[WONDROUS_PATCH]+[SACRED_ASH]+[HILDA]*3+
...
def _pick_discard_return(obs,opts):
    cur=obs.get('current') or{}; me=cur.get('yourIndex',0)
    players=cur.get('players') or[]; mine=players[me] if len(players)>me else{}
    discard=mine.get('discard') or[]
    def _rank(o):
        idx=o.get('index'); cid=_pk_id(discard[idx]) if (idx is not None and 0<=idx<len(discard)) else -1
        return {ABRA:5,KADABRA:4,ALAKAZAM:3}.get(cid, 2 if cid in PSYCHIC_ENERGY_IDS else 0)
    return [max(range(len(opts)),key=lambda i:_rank(opts[i]))]
...
def _choose(obs):
    ...
    if stype==1:
        ...
        if any(o.get('area')==3 for o in opts):

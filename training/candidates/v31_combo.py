...
def _main_phase_features(obs,sel):
    ...
    def score(o):
        ...
        if ot==PLAY:
            ...
            # v31 develop-before-KO. Attacking is what ENDS the turn, so any
            # development done before it is free -- the only real cost is that
            # Powerful Hand reads 20 x hand size, so each card spent is 20 less
            # damage. Previously ANY can_ko state crushed every PLAY to 1.0 and
            # we attacked immediately, which is how the board stays 1-2 wide:
            # board-thinning is this project's #1 documented live failure mode
            # (18/18 mined exploiter losses, 10/27 fresh v29d losses end with
            # zero Alakazam-line pieces in play). v30-exp's rescue mining already
            # measured development-before-KO at 19-22% rescue vs a 7.9% baseline
            # but gated it to a narrow late regime; disagreement mining vs
            # alakazam_v9 (778.2, byte-identical deck) shows it develops FIRST as
            # a matter of course -- ~3-4 such disagreements per game, the single
            # dominant class. Generalised here, guarded so lethality is kept.
            if can_ko and not rescue_hold_ko:
                _dev_ok = False
                if cid in BENCHABLE_BASIC_IDS and cen['bench_count'] < 5:
                    _need = -(-opp_hp // PH_DMG_PER_CARD)   # cards required for the KO
                    _dev_ok = (hand_n - 1) >= _need         # still lethal after the spend
                if not _dev_ok: return 1.0
...
def _choose(obs):
    ...
    if stype==1:
        ...
        if ctx==41:
            ...
            if no_i: return _clamp(no_i,sel)

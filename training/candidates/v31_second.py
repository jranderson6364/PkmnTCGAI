...
def _choose(obs):
    ...
    if stype==1:
        ...
        # v31: IS_FIRST (ctx 41) -- "would you like to go first?". This file had
        # NO handling for it, so it fell through to the blanket "prefer YES" below
        # and we went first every single game by accident, never by decision.
        # Disagreement mining vs alakazam_v9 (publicScore 778.2, byte-identical
        # deck, beats us 55%) found it answers NO 20/20 -- a 100% disagreement on
        # a once-per-game structural choice. Mechanism: Powerful Hand deals
        # 20 x hand size, and the player going second is one card up at every
        # comparable point of the game, so hand-size-as-damage prefers second.
        if ctx==41:

...
_COLLECT_PATH = os.environ.get("TWOPLY_COLLECT_LOG")
_GAME_ID = os.environ.get("TWOPLY_GAME_ID", "0")
if _COLLECT_PATH:
    ...
    if _COLLECT_PATH and heur_top in avg:
        try:
            ...
            rec = {"game": _GAME_ID, "seat": me_i, "turn": cur.get("turn"),
                   ...
                   "heur_top": int(heur_top), "search_best": int(best),
                   "leaf_mode": LEAF_MODE}
            with open(_COLLECT_PATH, "a") as _f:
                print(_json.dumps(rec), file=_f)
        except Exception:
            pass
    if best == heur_top:
        return None

...
def _search_decide(obs_dict):
    ...
    for _ in range(N_DET):
        if time.monotonic() > deadline:
            break
        try:
            _pf = lambda n: [DECK[random.randrange(len(DECK))] for _ in range(n)] if n>0 else []
            _me = cur["players"][me_i]; op = cur["players"][1 - me_i]
            zones = (_pf(_me.get("deckCount",0)), _pf(len(_me.get("prize") or [])),
                     _pf(op.get("deckCount",0)), _pf(len(op.get("prize") or [])),
                     _pf(op.get("handCount",0)),
                     _pf(1) if (op.get("active") and op["active"][0] is None) else [])
        ...
        try:
            for idx in cand:
                ...
                if (cs is None or (cs.result is not None and cs.result >= 0)
                    ...
                    acc[idx] += _leaf_eval(cs, me_i); n_eval[idx] += 1
                    continue
                sid1, c1 = _advance_forced(sid1, c1, 1 - me_i, deadline)
                cs = c1.current
                if (cs is None or c1.select is None
                        or c1.select.context != MAIN or cs.yourIndex == me_i):
                    acc[idx] += _leaf_eval(cs, me_i); n_eval[idx] += 1
                    continue
                d1 = _obs_dict(c1)
                op_base = H.score_options_main(d1, d1.get("select") or {})
                op_order = (sorted(range(len(op_base)), key=lambda i: -op_base[i])
                            if op_base else list(range(len(c1.select.option))))
                worst = None
                for k in range(min(K_OPP, len(op_order))):
                    ...
                    try:
                        ...
                        continue
                    sid2, c2 = ss2.searchId, ss2.observation
                    sid2, c2 = _greedy_complete_turn(sid2, c2, 1 - me_i, deadline)
                    sid2, c2 = _advance_forced(sid2, c2, me_i, deadline, limit=6)
                    v = _leaf_eval(c2.current, me_i)
                    worst = v if worst is None else min(worst, v)
                if worst is None:
                    worst = _leaf_eval(cs, me_i)
                acc[idx] += worst; n_eval[idx] += 1
        finally:
            try:
                search_end()
            except Exception:
